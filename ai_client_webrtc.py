...
# WebRTC相关配置
JANUS_URL = os.getenv("JANUS_URL", "ws://127.0.0.1:8188")
JANUS_API_SECRET = os.getenv("JANUS_API_SECRET", "janusrocks")
JANUS_PLUGIN = "janus.plugin.audiobridge"
AUDIO_ROOM_ID = 1234  # 音频房间ID
WEBRTC_RECONNECT_DELAY = 5  # 重连延迟（秒）
WEBRTC_MAX_RETRIES = 3  # 最大重试次数
...
class JanusSignaling:
    ...
    async def join_room(self, room_id, display_name="AI_Backend"):
        """加入音频房间"""
        self.transaction_id += 1
        message = {
            "janus": "message",
...
def on_audio_done(audio_base64):
    ...
    logger.info(f"📤 音频数据已放入发送队列: {len(audio_bytes)} 字节")
...
def process_audio(raw_audio_data, session_id=None):
    """处理音频数据的核心函数"""
...
async def process_audio_track(track):
    ...
    logger.info("开始处理音频轨道数据...")
    audio_buffer = b""
    buffer_size = 16000 * 2  # 1秒的16kHz 16位音频
        ...
        while True:
            ...
            if frame:
                ...
                if len(audio_buffer) >= buffer_size:
                    ...
                    logger.info(f"收到音频数据: {len(audio_buffer)} 字节")
                    audio_buffer = b""
                    
    except Exception as e:
        logger.error(f"处理音频轨道出错: {e}")
        import traceback
        logger.error(traceback.format_exc())

async def audio_processing_loop():
    """音频处理循环 - 从队列中获取音频数据并处理"""
    global audio_queue, response_queue
    ...
    while True:
        try:
            ...
            if not audio_queue.empty():
                raw_audio_data = audio_queue.get()
                logger.info(f"从队列中获取音频数据: {len(raw_audio_data)} 字节")
                
                # 处理音频数据
                audio_response, text_response = process_audio(raw_audio_data)
                
                # 如果有音频回复，放入发送队列
                if audio_response:
                    response_queue.put(audio_response)
                    logger.info(f"音频回复已放入发送队列: {len(audio_response)} 字节")
                
                # 记录文本回复
                if text_response:
                    logger.info(f"文本回复: {text_response}")
            ...
            import traceback
            logger.error(traceback.format_exc())
            await asyncio.sleep(1)  # 出错后等待更长时间

async def setup_webrtc_connection():
    """建立WebRTC连接"""
    global webrtc_connection
    
    try:
        # 创建RTCPeerConnection
        webrtc_connection = RTCPeerConnection()
        
        # 创建音频发送轨道并添加到连接
        audio_send_track = AudioSendTrack()
        webrtc_connection.addTrack(audio_send_track)
        
        # 设置轨道处理器 - 处理接收到的音频轨道
        @webrtc_connection.on("track")
        def on_track(track):
        ...
        # 设置连接状态监听器
        @webrtc_connection.on("connectionstatechange")
        async def on_connectionstatechange():
            logger.info(f"WebRTC连接状态: {webrtc_connection.connectionState}")
            if webrtc_connection.connectionState == "failed":
                logger.error("WebRTC连接失败，尝试重新连接")
                await handle_webrtc_reconnect()
        
        # 创建Janus信令连接
        signaling = JanusSignaling(JANUS_URL)
        if not await signaling.connect():
            logger.error("Janus信令连接失败")
            return False
        
        # 加入音频房间
        await signaling.join_room(AUDIO_ROOM_ID)
        
        # 创建offer
        offer = await webrtc_connection.createOffer()
        await webrtc_connection.setLocalDescription(offer)
        
        # 发送offer到Janus
        await signaling.send_offer(offer)
        
        # 接收answer
        answer = await signaling.receive_answer()
        if answer:
            await webrtc_connection.setRemoteDescription(answer)
...
async def start_webrtc_client():
    """启动WebRTC客户端"""
    signaling = None
    try:
        # 建立WebRTC连接
        signaling = await setup_webrtc_connection()
        if not signaling:
        ...
        logger.info("WebRTC客户端启动成功，等待音频数据...")
        ...
        logger.error(f"WebRTC客户端启动失败: {e}")
        import traceback
        logger.error(traceback.format_exc())
...
async def start_webrtc():
    """启动WebRTC连接"""
    global webrtc_connection, reference_audio_file
    
    try:
        # 启动WebRTC客户端
        success = await start_webrtc_client()
        if not success:
            logger.error("WebRTC客户端启动失败")
            return False
        
        return True
        
    except Exception as e:
        logger.error(f"启动WebRTC时出错: {e}")
        import traceback
        logger.error(traceback.format_exc())
        return False
...
def main():
    ...
    # 打印启动信息
    logger.info("=" * 50)
    logger.info("AI音频处理客户端启动 (WebRTC模式)")
    logger.info(f"Janus URL: {JANUS_URL}")
    logger.info(f"音频房间ID: {AUDIO_ROOM_ID}")
...
async def handle_webrtc_reconnect():
    """处理WebRTC重连逻辑"""
    global reconnect_attempt, reconnect_delay_seconds, max_reconnect_attempts
    
    while reconnect_attempt < max_reconnect_attempts:
        reconnect_attempt += 1
        logger.info(f"WebRTC连接断开，将在 {reconnect_delay_seconds} 秒后尝试重新连接... ({reconnect_attempt}/{max_reconnect_attempts})")
        await asyncio.sleep(reconnect_delay_seconds)
        
        try:
            success = await start_webrtc_client()
            if success:
                logger.info("WebRTC重连成功")
                reconnect_attempt = 0  # 重置重连计数
                return True
        except Exception as e:
            logger.error(f"WebRTC重连失败: {e}")
        
        # 指数退避算法增加重连延迟
        reconnect_delay_seconds = min(60, int(reconnect_delay_seconds * 1.5))
